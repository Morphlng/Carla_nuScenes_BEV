'''
Author: Morphlng
Date: 2023-08-29 13:23:18
LastEditTime: 2023-08-31 14:57:04
LastEditors: Morphlng
Description: This file is modified from <https://github.com/cjy1992/gym-carla/blob/master/gym_carla/envs/render.py>, Copyright (c) 2019: Jianyu Chen (jianyuchen@berkeley.edu).
FilePath: \BEV_render\birdeye_render.py
'''
...
class Util(object):
    ...
    @staticmethod
    def color_by_yaw(agent_yaw_in_radians: float, lane_yaw_in_radians: float):
        """
        ...
        angle = Util.angle_diff(agent_yaw_in_radians,
                                lane_yaw_in_radians, 2*np.pi) + np.pi
    ...
    def fade_color(color: "tuple[int, int, int]",
        ...
        new_rgb = colorsys.hsv_to_rgb(float(hsv_color[0]),
                                    float(hsv_color[1]),
                                    new_value * 255.)
        return [int(val) for val in new_rgb]
...
class MapImage(object):
    ...
    def __init__(self, carla_world, carla_map, pixels_per_meter):
        ...
        self.scale = 1.0
        self.id = f"{carla_map.name.split('/')[-1]}_{pixels_per_meter}_{self.scale}"
    ...
    def draw_road_map(self, map_surface, carla_world, carla_map, world_to_pixel, world_to_pixel_width):
        ...
        def draw_topology(carla_topology, index):
            ...
            if os.path.exists(os.path.join(os.path.dirname(__file__), f'birdeye_cache/{self.id}.png')):
                topology_map_surface = pygame.image.load(f'birdeye_cache/{self.id}.png')
                map_surface.blit(topology_map_surface, (0, 0))
            else:
                ...
                # cache the map surface to disk
                os.makedirs(os.path.join(os.path.dirname(__file__), 'birdeye_cache'), exist_ok=True)
                pygame.image.save(map_surface, f'birdeye_cache/{self.id}.png')
...
class BirdeyeRender(object):
    ...
    def _render_hist_actors(self, surface, actor_polygons, actor_type, world_to_pixel, num):
        """Render the history of the actors
        
        Args:
            surface (pygame.Surface): Surface to draw on
            actor_polygons (list[dict[id, polygon]]): List of actor polygons
            actor_type (str): Type of actor
            world_to_pixel (function): Function to convert world coordinates to pixel coordinates
            num (int): Number of history to render
        """
        ...
        for i in range(max(0, lp-num), lp):
            for ID, poly in actor_polygons[i].items():
                corners = [world_to_pixel(carla.Location(x=p[0], y=p[1])) for p in poly]
                corners.append(world_to_pixel(carla.Location(x=poly[0][0], y=poly[0][1])))
                ...
                if ID == self.hero_id:
                    ...
                    if actor_type == 'vehicle':
                        ...
                        base_color = (255, 153, 51)  # brown

                # Only fade the colors if there is a history
                if lp > 1: 
                    color = Util.fade_color(base_color, i, lp - 1)
                else:
                    color = base_color

                pygame_color = pygame.Color(*color)
                pygame.draw.polygon(surface, pygame_color, corners)


    def render_waypoints(self, surface, waypoints, world_to_pixel):
        vehicle_yaw_radians = math.radians(self.hero_transform.rotation.yaw)

        for i in range(len(waypoints) - 1):
            x1, y1, waypoint_yaw1 = waypoints[i]
            x2, y2, waypoint_yaw2 = waypoints[i+1]

            color = Util.color_by_yaw(
                vehicle_yaw_radians, math.radians(waypoint_yaw1))
            pygame_color = pygame.Color(*color)

            start_pixel_location = world_to_pixel(carla.Location(x=x1, y=y1))
            end_pixel_location = world_to_pixel(carla.Location(x=x2, y=y2))

            # 10 is the line thickness, adjust as needed
            pygame.draw.line(surface, pygame_color,
                             start_pixel_location, end_pixel_location, 10)
    ...
    def render(self, display, render_types=None):
        ...
        if isinstance(self.waypoints[0], list):
            for road in self.waypoints:
